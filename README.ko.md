...
| 층 | 커맨드 |
|---|---|
| L0 발견 | `root`(tri-state: repo / not-repo / error), `init`(멱등, `-b main`), `clone`(대상명 검증·progress) |
| L1 상태 | `status`(porcelain v2 `--branch`: 브랜치·upstream·ahead/behind·데코레이션 분류·`truncated` 상한), `watch.start` / `watch.stop` / `watch.list` |
| L2 워크트리 | `worktree.add`(`--no-track -b`, base 를 `soksak.worktree.<branch>.base` 로 박제), `worktree.list`, `worktree.remove`(더러운 트리 거부), `worktree.remove.force`(destructive), `worktree.prune` |
| L3 쓰기 | `stage`, `unstage`, `commit`, `discard` — 전부 destructive 선언. `discard` 는 전 경로가 repo 내부임을 증명해야만 untracked 파일을 삭제 |

모든 커맨드는 `sok plugin.soksak-plugin-git-core.<name>` 으로 실행합니다.
`path` 는 활성 프로젝트 루트로 폴백합니다. 읽기 삼종 `log` / `show` / `diff` 는
레거시 코어 표면과 동형이라 소비 플러그인이 동작 변화 없이 재배선할 수 있습니다.
